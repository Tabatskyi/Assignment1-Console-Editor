# Assignment 1 Console Editor Test Plan

Start the program, and load the sherlock.txt file with command 'l'. Print result with command 'p'. The program features dynamic memory array, so we expect resizing of memory.
...
Save file to the 'sherlock.txt'. File saved.
...
Also program features the clear command 'c' and quit 'q'.
